...
Benchmark implementations live under `tooling/suites/benchmarks/`; see [benchmark commands and evidence](tooling/suites/benchmarks/README.md). `npm run bench` measures CPU workloads, not GPU time or universal frame-rate guarantees. `npx tsx tooling/suites/benchmarks/rendering/browser-baseline.ts` drives sustained game runs in visible Chromium. For the fixed WebGPU renderer workload, first run `npm run build:browser`, then run the same baseline command with `NGNE_URL=http://127.0.0.1:4173/tooling/suites/benchmarks/rendering/browser/index.html?workload=renderer-webgpu`, `NGNE_SERVE_DIR=.`, and `NGNE_SERVE_OUT_DIR=dist-browser`. The renderer fixture records CPU preparation and submission only; it does not wait for GPU completion. Browser benchmark pages must remain visible because hidden pages throttle `requestAnimationFrame`.
