  ...
  diagnostic: (error) => {
    if (error instanceof Error) {
      ...
      el("error").textContent =
        `Flight interrupted: ${error.message}. Reload to try again.`;
      console.error(error);
    }
  },
...
function updateUI(stats: Readonly<Stats>) {
  ...
  [...el("hull").children].forEach((node, i) =>
    node.classList.toggle("empty", i >= view.hp),
  );
  el("hull").setAttribute("aria-label", `${view.hp} of 5 hull points`);
  el("charge").style.width = `${(1 - view.bomb / 900) * 100}%`;
  el("nova-state").textContent = view.bomb
    ? `${Math.ceil(view.bomb / 60)} SEC`
    : "READY";
  el("combo").textContent =
    view.combo > 1
      ? `${view.combo}× CHAIN / KEEP IT GOING`
      : "AUTO-FIRE / ONLINE";
  ...
  pause.innerHTML =
    phase === "paused" ? "Resume <kbd>P</kbd>" : "Pause <kbd>P</kbd>";
  ...
  if (performance.now() - metricTime > 200) {
    ...
    el("graph-path").setAttribute(
      "d",
      times
        .map((v, i) => `${i ? "L" : "M"}${i * 2} ${29 - Math.min(28, v * 2)}`)
        .join(" "),
    );
  }
}
