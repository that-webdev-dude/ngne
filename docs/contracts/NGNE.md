...
`Query.each()` are a deprecated bridge with no remaining game consumers: NGNE-27 has migrated
Starfall and the platformer and removes the bridge next. Schema and legacy components cannot
...
| ECS values and identity                 | World owns component columns and immutable index/generation/world handles. Component names identify composition; conflicting definition objects with the same name are rejected on spawn. Names are world-local, not a global schema registry. Schema definitions are frozen at creation and hold cloned, frozen field descriptors (`kind`, `default`), so they are immutable authoring, not state. Schema values live in per-chunk typed columns; entity references are encoded index/generation pairs. Query chunk descriptors, `views` and `entityAt()` row meanings are borrowed for the current commit epoch and rebuilt on the first traversal after each commit: derived, never authority. Legacy object columns keep their bridge meaning until the bridge is removed.                                                                                                                                                                                                                                                |
| Allocator and iteration history         | World owns slot generations, row positions, pending flags, free-stack order, archetype creation order and dense row order. Schema archetypes also own chunk creation order and fill: allocation reuses the lowest-created chunk with capacity and empty chunks are retained, so chunk order and counts are history. A live schema slot's location is its `chunk` index plus chunk-relative `row`; legacy slot rows are archetype-relative. Inspection includes `archetypes` with ordered component names and entity indices, **including empty archetypes**; schema archetypes add `fields` (per component, field `name`/`kind`/`default`) and `chunks` (`capacity`, `count`, ordered entity indices). `entities` retains values in archetype/chunk/row order: legacy components as `{ name, value }`, schema components as field records `fields: [{ name, kind, value }]`, with entity references as `null` or `{ index, generation }`. Empty archetypes and empty chunks cannot be reconstructed from live entities alone. |
...
`Query.each()` until NGNE-27 removes it; neither game uses it any longer. Empty `spawn()` remains a
...
WebGPU rejects with an actionable message and never silently falls back. Hello and both
games opt in; NGNE-27 removes the option and the WebGL default next.
