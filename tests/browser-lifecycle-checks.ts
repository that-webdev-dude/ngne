...
export async function checkBrowserLifecycle(
...

    {
        ...
        await app.stop();
        check(
            JSON.stringify(app.game.enumerate()) === JSON.stringify(before),
            "stop preserves scene identity, resources, RNG, freeze and committed state",
        );
        ...
        check(
            callbacks.length === count &&
                JSON.stringify(app.game.enumerate()) === JSON.stringify(before),
            "repeated start and stop during resume reject without corruption; old callbacks stay invalid after restart",
        );
        ...
        check(
            ...
            "start and repeated stop during suspension reject without corrupting stop",
        );
        await app.dispose();
    }
    ...
    for (const rejectResume of [false, true]) {
        ...
        check(
            app.game.lifecycle === "Disposed" &&
                app.game.scenes.length === 0 &&
                disposal === app.dispose(),
            "disposal tears down synchronously and repeated calls share completion",
        );
        ...
        check(
            ...
            `late resume ${rejectResume ? "failure" : "success"} cannot resurrect a disposed host`,
        );
    }
...

    {
        ...
        check(
            app.game.lifecycle === "Failed" && JSON.stringify(app.game.enumerate()) === before,
            "audio resume failure preserves mounted state for disposal",
        );
        await app.dispose();
    }
    ...
    for (const incomplete of [false, true]) {
        ...
        check(
            fixture.app.game.lifecycle === (incomplete ? "Failed" : "Stopped") &&
                fixture.app.game.scenes.length === 0,
            `cold startup ${incomplete ? "incomplete" : "complete"} rollback retains its failure semantics`,
        );
        await fixture.app.dispose().catch((error: unknown) => {
...

    {
        ...
        check(
            fixture.app.game.lifecycle === "Failed" && fixture.app.game.scenes[0].id === id,
            "resume scheduler failure preserves mounted scene",
        );
...

    {
        ...
        check(
            app.game.lifecycle === "Disposed",
            "late stop failure cannot overwrite terminal disposal",
        );
    }

    {
        ...
        fixture.failCancel();
        fixture.app.audio.dispose = async () => {
            attempted.push("audio");
