...
export async function checkInterpolation(
    ...
    const sample = (
        ...
        check(
            JSON.stringify(game.enumerate()) === before,
            `${label}: rendering preserves simulation`,
        );
    };
    try {
        await game.start(await game.prepare(definition, { key: "room" }));
        check(
            JSON.stringify(game.enumerate().scenes[0].resources) ===
                JSON.stringify({
                    actor: { x: 40, previousX: 40 },
                    effect: { x: 80, previousX: 80 },
                }),
        ...
        const modal = await game.prepare(
        ...
        check(
            JSON.stringify(game.enumerate().scenes[0]) === suspended,
            "Suspension preserves all scene state",
        );
        game.pop();
        game.tick();
        ...
        await game.start();
        check(
            JSON.stringify(game.enumerate()) === stopped,
            "Stop/resume preserves all simulation state",
        );
