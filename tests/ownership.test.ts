...
test("completed inspection retains empty archetype order and allocator reuse", async (t) => {
    ...
    const scene: SceneDefinition = {
        ...
        setup(s) {
            const handles = s.resource("handles", {
                first: s.world.spawn(A.of()), second: s.world.spawn(B.of()),
            });
    ...
    assert.deepEqual(empty.scenes[0].world, {
        archetypes: [{ components: ["a"], entities: [] }, { components: ["b"], entities: [] }],
        slots: [
    ...
    assert.deepEqual(populated.archetypes, [
        { components: ["a"], entities: [0] }, { components: ["b"], entities: [1] },
    ]);
...
test("showcase inspection accounts for owners without render or stop/resume mutation", async (t) => {
    const create = () => new Game<Progress, ProgressCommand>({
    ...
    assert.ok(scene.world.entities.some(entity =>
        entity.index === player.index && entity.generation === player.generation));
