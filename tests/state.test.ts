...
interface Progress { scores: number[]; nested: { best: number } }
interface Command { values: number[] }
...
test("initial and transition snapshots copy and deeply freeze shallow-frozen containers", async () => {
    ...
    await game.start(await game.prepare({ id: "writer", setup(s) {
        const state = s.state();
...
test("plain data preserves cycles, aliases, sparse arrays and primitive values", () => {
    interface Node { children: Node[]; value: unknown; self?: Node }
    const root: Node = { children: [], value: { absent: undefined, nil: null, big: 2n, nan: NaN, infinity: Infinity } };
    root.self = root;
...
test("all durable boundaries reject unsupported values even inside frozen objects and arrays", async () => {
    ...
    const unsupported: unknown[] = [
        new Map(), new Set(), new Date(), /x/, new Uint8Array(), new ArrayBuffer(0),
        new Custom(), () => 1, Symbol("value"),
        Object.defineProperty({}, "hidden", { value: 1 }),
        { [Symbol("key")]: 1 },
        { get value() { throw new Error("Getter must never run"); } },
        { get then() { throw new Error("Getter must never run"); } },
    ];
    for (const value of unsupported) {
        const payload = Object.freeze({ nested: Object.freeze([value]) });
        assert.throws(() => new Game({ seed: 1, state: payload, transition: s => s }), /plain data|plain objects|data properties/);
        const game = new Game<unknown, unknown>({ seed: 1, state: {}, transition: () => payload });
        await game.start(await game.prepare({ id: "writer", setup(s) {
            const state = s.state();
            s.system(() => {
                assert.throws(() => state.dispatch(payload), /plain data|plain objects|data properties/);
...
test("commands are copied on dispatch, frozen for transitions and applied in order", async () => {
    ...
    const game = new Game<Progress, Command>({
        seed: 1, state: { scores: [], nested: { best: 0 } },
    ...
    const scene: SceneDefinition<Progress, Command> = { id: "writer", setup(s) {
        const state = s.state();
        s.system(() => {
...
test("selected scenes share the tick snapshot and replacement setup reads committed results", async () => {
    ...
    const game = new Game({ seed: 1, state: { value: 0 }, transition: (s, c: number) => ({ value: s.value * 10 + c }) });
    const writer = (id: string, value: number): SceneDefinition<{ value: number }, number> => ({ id, setup(s) {
        const state = s.state();
        assert.throws(() => state.dispatch(9), /active system update/);
        s.system(() => { reads.push(state.read()); state.dispatch(value); });
    } });
    ...
    reads.length = 0;
    const before = game.state;
    const replacement = await game.prepare({ id: "replacement", setup(s) {
...
test("retained state access cannot dispatch outside its owning system update", async () => {
    ...
    const game = new Game({ seed: 1, state: { value: 0 }, transition: (s, c: number) => {
        rejectDispatch();
...
test("asynchronous transition results fail without publishing or leaving an unhandled rejection", async () => {
    ...
    const game = new Game<unknown, undefined>({ seed: 1, state: {},
        transition: () => Promise.reject(failure), diagnostic: error => errors.push(error),
    });
    await game.start(await game.prepare({ id: "writer", setup(s) {
        const state = s.state();
        s.system(() => state.dispatch(undefined));
...
test("events use the same plain-data validation and retain an owned frozen payload", async () => {
    ...
    await game.start(await game.prepare({ id: "events", setup(s) {
        s.system(ctx => {
            if (ctx.simulationTick === 0) {
                assert.throws(() => ctx.emit({ type: "bad", nested: Object.freeze({ map: new Map() }) }), /plain objects/);
                ctx.emit({ type: "ok", payload });
                payload.scores.push(2);
            } else {
                assert.deepEqual(ctx.events, [{ type: "ok", payload: { scores: [1] } }]);
...
test("a later invalid transition preserves earlier committed commands and fails the Game", async () => {
    const game = new Game<unknown, number>({ seed: 1, state: 0,
        transition: (_, command) => command === 1 ? 1 : Object.freeze({ nested: new Set() }),
    });
    await game.start(await game.prepare({ id: "writer", setup(s) {
        const state = s.state();
        s.system(() => { state.dispatch(1); state.dispatch(2); state.dispatch(3); });
    } }, { key: "writer" }));
