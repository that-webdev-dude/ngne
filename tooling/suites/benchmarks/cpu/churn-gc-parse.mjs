...
/** @typedef {{isolate:string,ms:number,type:string,pauseMs:number,beforeMB:number,afterMB:number}} TraceEvent */
/** @typedef {{startTime:number,duration:number,kind:number}} ObserverEntry */
/** @type {TraceEvent[]} */
const events = [];
let unmatched = 0;
...
if (
    ...
    throw Error("Invalid GC observer entries");
const observed = entries.filter((e) => e.kind !== INCREMENTAL_KIND);
const candidates = [];
